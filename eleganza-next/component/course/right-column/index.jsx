...
export default function Rightcolumn({ filters, sortOrder, searchCourseList }) {
      ...
      return Object.entries(filters).every(([key, value]) => {
        ...
        if (key === 'dateSearchRange') {
          ...
          if (!!startDate && !!endDate ) {
            ...
            return courseStartDate >= startDateObj && courseStartDate <= endDateObj;
          }
          return true;
        }
        if (key === 't_years') {
  ...
  const updateCourses = (courseData, teachersData, filters, sortOrder, currentPage, coursesPerPage) => {
    ...
    const currentCourses = paginateCourses(filteredCourses, currentPage, coursesPerPage);
    return { totalCourses, currentCourses };
  };
  
  useEffect(() => {
    const { totalCourses, currentCourses } = updateCourses(courseToFilter, teachersData, filters, sortOrder, currentPage, coursesPerPage);
